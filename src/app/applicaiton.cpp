...
void Application::run()
{
    ...
    glm::mat4 projectionMatrix = Math::createProjectionMatrix(glm::vec2(m_window.ScreenWidth(), m_window.ScreenHeight()), 90, 0.1, 1000);
    ...
    chunk_manager.generate(1, 1, 1, &atlas);
    ...
    while (running)
    {
        ...
        while (m_window.getWindow()->pollEvent(e))
        {
            ...
            {
                wireframe = !wireframe;
                if (wireframe) m_window.EnableWireframe(true);
                else           m_window.EnableWireframe(false);
            }

            // Temporary block changing method
            if (e.type == sf::Event::KeyPressed && e.key.code == sf::Keyboard::B)
            {
                if (selectedBlock == 6) selectedBlock = 1;
                else selectedBlock++;
            }
        ...
        player.Update(&chunk_manager, elapsed, m_window.getWindow(), 7.5f);
